...
p = [0.2  0.7]; %0.51*rand(1,2);
for j=1:3    
    data{referenceDemo}(:,j) = fakeQuery(data{referenceDemo}(:,j)', p, 1)';
    subplot(3,1,j);
    plot(data{referenceDemo}(:,j), sty([0.5 0.5 1], [], 3)  );
end
% Chose which DoF to use as reference [1:x, 2:y, 3:z] for alignment. The other
% DoFs will be aligned as according to the solution found for the reference
